...
export function App({initial}:{initial?:Dashboard}){
  ...
  const warningsFor=(site:Site)=>data.warnings.filter(item=>item.site_codes?.includes(site.code)||site.warningAreaCodes?.includes(item.area_code));
  ...
  return <main>
    ...
    <section className="viewport" aria-live="polite">
      {page===1?<div className="page page1">
        {!data.sites.length||!hasWeather?<Empty last={data.status.lastSuccessfulReceiveAt} error={loadError}/>:<div className="cards">{data.sites.map(site=>{const observation=observations[site.code]||{};const forecast=forecastFor(site.code);const warnings=warningsFor(site);const warningText=warnings.length?warnings.slice(0,2).map(item=>`${item.level} ${item.phenomenon}`).join(' · '):site.warningAreaCodes?.length||data.warnings.some(item=>item.site_codes?.length)?'발효 특보 없음':'특보 매핑 미설정';return <article className="card" key={site.code}><div className="title"><h2>{site.name}</h2><code>{site.code}</code></div><div className="temp">{observation.temperature??'--'}<small>°C</small></div><dl><div><dt>습도</dt><dd>{observation.humidity??'--'}%</dd></div><div><dt>바람</dt><dd>{observation.wind_direction||'--'} {observation.wind_speed??'--'}m/s</dd></div><div><dt>순간풍속</dt><dd>{observation.gust_speed??'--'}m/s</dd></div><div><dt>강수</dt><dd>{observation.precipitation_state||'--'} {observation.precipitation??'--'}mm</dd></div><div><dt>하늘</dt><dd>{observation.sky||forecast?.sky||'--'}</dd></div><div><dt>오늘 최저/최고</dt><dd>{forecast?.min_temperature??'--'} / {forecast?.max_temperature??'--'}°</dd></div></dl><div className="warning">{warningText}</div><time>기준 {formatTime(observation.observed_at||forecast?.valid_at)}</time></article>})}</div>}
