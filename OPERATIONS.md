# 운영 지침
- 상태: `/health/live`는 프로세스, `/health/ready`는 DB 읽기·쓰기와 초기화 상태입니다. 데이터 지연은 `/api/v1/status`의 `normal/stale/disconnected`로 별도 감시합니다.
...
- directory: 10초 간격으로 `*.json`을 처리해 성공은 삭제, 실패는 `/data/quarantine`으로 이동합니다.
