...
func (s *Store) init(sitesPath string) error {
	...
	var sites []Site
	if e = yaml.Unmarshal(b, &sites); e != nil {
		return e
	}
	tx, e := s.DB.Begin()
	if e != nil {
...
func (s *Store) Ready(ctx context.Context) error {
	...
	defer tx.Rollback()
	if _, e = tx.ExecContext(ctx, "CREATE TEMP TABLE IF NOT EXISTS readiness(v INTEGER)"); e != nil {
		return e
	}
	_, e = tx.ExecContext(ctx, "INSERT INTO readiness VALUES(1)")
	return e
}
